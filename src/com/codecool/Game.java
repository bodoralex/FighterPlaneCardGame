...
public class Game {
	...
	public void setUpServer() {
		...
		serverGUI.printToTextArea("megy");
		serverGUI.printToTextArea(playersNumber + " " + robotsNumber);
	...
	public void setPlayers() {
		...
		broadCast("SEND", namelessPlayers);
		...
		serverGUI.printToTextArea((Arrays.toString(this.players.stream().map(x -> x.getName()).toArray())));
	}
	...
	public void awardWinner(TreeMap<Card, PlayCapable> map) {
		...
		serverGUI.printToTextArea("The winner got " + count + " cards");
	}

	public PlayCapable round() {
		serverGUI.printToTextArea(border);
		broadCast(roundNumber);
		Map<Card, PlayCapable> cards = new HashMap<>();
		PlayCapable roundAttacker = roundAttacker();
		serverGUI.printToTextArea(roundAttacker.getName() + " is attacking!\n");
		int choice = roundAttacker.choose();
		serverGUI.printToTextArea(
				String.format("%s' choosen arrtibute is: %s", roundAttacker.getName(), attrs[choice - 1]));
		for (PlayCapable player : players) {
			serverGUI.printToTextArea(player.getName() + "'s card: " + player.peek());
		...
		for (PlayCapable player : players) {
			serverGUI.printToTextArea(
					String.format("%s has %d card(s) left", player.getName(), player.cardsRemaining()));
		}
	...
	private void broadCast(Object object, ArrayList<Player> players) {
	...
	public void deal(Deck deck) {
		//serverGUI.printToTextArea("How many cards do you want to play with?");
		//serverGUI.printToTextArea("The maximum card number is: " + deck.deckSize());
		Integer cardsNumber = new Integer(10);
		while (cardsNumber.equals(0)) {
			try {
				...
				while (cardsNumber > 40 || cardsNumber < 1 || cardsNumber == -1 || cardsNumber < players.size()) {
					serverGUI.printToTextArea("Wrong number input! Try again!");
					howMany = Main.scanner.next().trim();
					cardsNumber = Integer.parseInt(howMany);
				}
			} catch (Exception e) {
				serverGUI.printToTextArea("That is not an integer.");
			}
		}
		deck.handout(getPlayers(), cardsNumber);
		serverGUI.printToTextArea("Cards are dealt.");
	}
	...
	public PlayCapable outOfTheGame() {
		...
		while (it.hasNext()) {
			...
			if (player.cardsRemaining() == 0) {
				serverGUI.printToTextArea(player.getName() + " has zero cards left and is out of the game.");
				it.remove();
			}
		}
		if (players.size() == 1) {
	...
	public void play() {
		...
		serverGUI.printToTextArea("The game is ended. The winner is: " + outOfTheGame().getName());
		System.exit(0);
	}
}
